...
const SideBar = () => {
  ...
  const fetchUserData = async (accId, accessToken) => {
    try {
      ...
      notifyErrorWithCustomMessage(
        "Lỗi kết nối khi lấy thông tin người dùng",
        messageApi
      );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        handleHttpStatusCode(
          response.status,
          "",
          `Lấy chi tiết khách hàng thất bại: ${
            errorText || response.statusText
          }`,
          messageApi
        );
      ...
      if (!data) {
        notifyErrorWithCustomMessage(
          "Không tìm thấy chi tiết khách hàng",
          messageApi
        );
        return null;
      }
      ...
      notifyErrorWithCustomMessage(
        "Lỗi kết nối khi lấy chi tiết người dùng",
        messageApi
      );
      console.error("Error fetching user details:", error);
      return null;
  ...
  const menu_customer = [
    ...
    { key: "chatbot", label: "Hỗ trợ", icon: <MessageOutlined /> },
    {
      key: "terms-service",
    ...
    { key: "contact-service", label: "Liên hệ", icon: <PhoneOutlined /> },
